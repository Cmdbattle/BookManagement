...
class validRegister {
    public static function confirm($firstName, $lastName, $email, $password) {
        ...
        try{
            $user = bookDB::getUserByEmail($email);
            $error_message = 'Error';
    ...
    public static function validLogin($email, $password) {
        $error_message = "";
        if ($email === '') {
            $error_message = 'Error';
            ...
            try {
            $success = bookDB::getUserByEmail($email);
        ...
        if ($password != '' && isset($success))
           {          
            try {
                $db_password = bookDB::getPassword($email);
